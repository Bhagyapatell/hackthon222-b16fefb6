...
const PurchaseOrderList = () => {
  const { data: orders, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('purchase_orders').select('*, contacts(name)').eq('is_archived', false).order('created_at', { ascending: false });
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/purchase-orders/new"><Button><Plus className="h-4 w-4 mr-2" />New Order</Button></Link>
      </div>
          ...
          {orders?.map((order) => (
            <Link key={order.id} to={`/purchase-orders/${order.id}`}>
              <Card className="hover:border-primary cursor-pointer">
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground">Vendor: {order.contacts?.name}</p>
